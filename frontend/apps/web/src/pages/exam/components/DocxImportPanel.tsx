...
// ─── Types（对齐后端 DocxImportResultVO / QuestionImportItemVO）────────────────
...
// ─── API ──────────────────────────────────────────────────────────────────
// 注意：http 拦截器已解包 Result→data。后端为单步导入（无 preview/confirm 两步）。

const docxApi = {
  importDocx: (bankId: number, file: File, defaultScore = 2): Promise<ImportResult> => {
    const formData = new FormData()
    formData.append('file', file)
    return http.post(`/v1/exam/banks/${bankId}/import/docx`, formData, {
...
export function DocxImportPanel({ bankId, bankName, onClose }: Props) {
  ...
  const importMutation = useMutation({
    mutationFn: (file: File) => docxApi.importDocx(bankId, file),
    onSuccess: (res) => {
      setResult(res ?? null)
  ...
  const errorRows = result?.details.filter((r) => !r.success) ?? []
  const displayRows = filterError ? errorRows : (result?.details ?? [])

  return (
    <div className="flex flex-col gap-5">
      {/* 步骤说明 */}
      <div className="flex items-center gap-2 text-xs text-gray-500">
        <StepDot n={1} active={step === 'select'} done={step === 'done'}>选择文件</StepDot>
        <div className="h-px flex-1 bg-gray-200" />
        <StepDot n={2} active={step === 'done'} done={false}>导入结果</StepDot>
      </div>
      ...
      {step === 'select' && (
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3">
            ...
            >
              {importMutation.isPending ? '导入中...' : '解析并导入'}
            </button>
          </div>
          {importMutation.isError && (
            <p className="text-xs text-red-500">导入失败，请检查文件格式或稍后重试</p>
          )}
        </div>
      )}
      ...
      {step === 'done' && result && (
        <div className="space-y-4">
          <div className="flex gap-4">
            <Stat label="识别总数" value={result.total} color="text-gray-700" />
            <Stat label="成功导入" value={result.successCount} color="text-green-600" />
            <Stat label="失败跳过" value={result.failCount} color="text-red-600" />
          </div>
          ...
          {errorRows.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              >
                {filterError ? '显示全部' : `仅看失败行 (${errorRows.length})`}
              </button>
              <p className="text-xs text-gray-400">失败行已跳过，未导入</p>
            </div>
          )}

          <div className="max-h-80 overflow-y-auto space-y-2 rounded-lg border border-gray-200 p-2">
            {displayRows.map((row) => (
              <ParsedRowCard key={row.index} row={row} />
            ))}
          </div>

          <div className="flex items-center justify-end gap-3">
            <p className="mr-auto text-sm text-gray-500">题目已添加至「{bankName}」</p>
            <button
              type="button"
              onClick={onClose}
...
function ParsedRowCard({ row }: { row: ImportItem }) {
  ...
  return (
    <div className={`rounded-lg border p-2.5 ${hasError ? 'border-red-300 bg-red-50' : 'border-gray-100 bg-white'}`}>
      <div className="flex items-start justify-between gap-2">
        <span className="text-xs text-gray-400">第 {row.index} 题</span>
        {hasError ? (
          <span className="flex-shrink-0 rounded-full bg-red-100 px-2 py-0.5 text-xs text-red-600 font-medium">失败</span>
        ) : (
          <span className="flex-shrink-0 rounded-full bg-green-100 px-2 py-0.5 text-xs text-green-600 font-medium">✓ 导入</span>
        )}
      </div>
      {hasError ? (
        <p className="mt-1.5 text-xs text-red-600">{row.errorMessage}</p>
      ) : (
        <p className="mt-1 text-xs text-gray-700 line-clamp-2">{row.contentPreview}</p>
      )}
    </div>
  )
}
